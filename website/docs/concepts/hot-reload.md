...
- **New or changed Redis services.** The engine dials them during step 2 and retries with the `startup_initial_interval` / `startup_max_interval` back-off until `PING` succeeds or `startup_max_elapsed_time` has passed. The default `startup_max_elapsed_time` is `0s`, which means retry forever: a reload that adds an unreachable Redis service waits until Redis is reachable, and later reloads queue behind it. Set `startup_max_elapsed_time` (for example `30s`) on services that may be unavailable. `HyperRedis` does not expose this field, so operator-managed services use the default.
